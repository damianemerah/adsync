...
- **Smart Links**: Universal redirects (`sellam.app/l/[token]`) for both WhatsApp and Website destinations.
- **Sellam Pixel**: Lightweight 1x1 GIF pixel (`/api/pixel`) for tracking website views and purchases.
...
3.  **Generation**: AdSync calls Flux.1 via Fal.ai to generate variations (Square/Portrait/Landscape) based on the "Simplicity" or "Creative" advantage.
...
1.  **Link Injection**: At campaign launch, the destination URL (WhatsApp link or Website URL) is wrapped in a Sellam smart link.
2.  **User Click**:
    - User clicks the ad → hits `sellam.app/l/[token]`.
...
4.  **WhatsApp Sales (Manual)**:
    ...
    - Merchant clicks "Sold! 🎉" in AdSync dashboard to manually record the sale and revenue.
