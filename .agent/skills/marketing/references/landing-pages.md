# Landing Page Copy Patterns

Proven formulas and examples for Tenzu landing page sections.

## Hero Section Formula

### Structure
```
[Outcome Promise] + [Time/Ease Qualifier] + [Key Differentiator]
```

### Examples

**Payment-Focused:**
```
Run Professional Facebook Ads in 2 Minutes
Pay in Naira. No Expert Needed. No Dollar Card Drama.

[CTA]: Start Free Trial →
```

**Creative-Focused:**
```
Turn Phone Photos Into Studio-Quality Ads
AI Does the Design. You Do the Business.

[CTA]: Create Your First Ad →
```

**Simplicity-Focused:**
```
Launch Facebook Campaigns Without the Confusion
Chat. Click. Launch. Your AI Ad Consultant Handles the Rest.

[CTA]: Try It Free →
```

### Hero Subheading Formula
```
[Specific Problem] + [Specific Solution] + [Proof Point]
```

**Examples:**
- "No designer? No problem. Over 2,000 Nigerian vendors create professional ads daily with Tenzu."
- "Card declined by Wask? Tenzu accepts your local card. Zero payment friction."
- "Confused by Ads Manager? Tenzu's AI builds campaigns you actually understand."

## Problem-Agitation-Solution (PAS) Section

### Formula
1. **Problem:** State the pain clearly
2. **Agitate:** Make them feel it
3. **Solution:** Show Tenzu as the fix

### Example 1: Payment Pain

**Problem:**
> "You've tried Wask. You've tried Canva Pro. Same story: 'Payment Declined.'"

**Agitate:**
> "It's not your card—it's their international gateway. While you struggle with payment forms, your competitors are already running ads."

**Solution:**
> "Tenzu accepts Naira payments via Paystack and bank transfer. Your local card works perfectly. Launch ads today, not someday."

[CTA]: Pay the Nigerian Way →

### Example 2: Creative Pain

**Problem:**
> "You have great products. Your photos? Not so great."

**Agitate:**
> "You know your ads look amateur. You've seen competitor ads that pop—clean backgrounds, professional lighting, compelling angles. Hiring an agency? ₦150,000 minimum. Hiring a designer? ₦50,000 per design. And that's if they even understand your vision."

**Solution:**
> "Tenzu's AI Studio transforms your phone photos into professional ads in seconds. No designer. No agency. No ₦150k invoice. Just upload, click 'Improve,' and watch AI work its magic."

[CTA]: Try AI Studio Free →

### Example 3: Complexity Pain

**Problem:**
> "Meta Ads Manager feels like piloting a spaceship."

**Agitate:**
> "50 fields. Campaign objectives, ad sets, placements, bidding strategies, custom audiences—where do you even start? You've watched 10 YouTube tutorials and you're still confused. One wrong setting and you've wasted ₦20,000 on the wrong audience."

**Solution:**
> "Tenzu's AI Consultant asks simple questions: Who's your customer? What's your budget? Then it builds the campaign for you. Chat-style. No confusing dashboards. Just clear, guided setup."

[CTA]: Build Your First Campaign →

## Feature-Benefit-Proof (FBP) Section

### Formula
```
[Feature Name]
[What It Does] → [Why That Matters] → [Social Proof]
```

### Example: AI Studio

**Feature:** AI Creative Studio

**What It Does:**
> Transforms your product photos into professional ads with AI-generated backgrounds, improved lighting, and design enhancements.

**Why That Matters:**
> Skip the ₦150k agency fee. Launch campaigns with studio-quality creatives today—not next week.

**Proof:**
> "I used to spend ₦200k per campaign on agency creatives. Now I create 10+ ad variations monthly for ₦15k total." - Chioma O., Lagos Fashion Vendor

[CTA]: See AI Studio in Action →

### Example: Naira Payment

**Feature:** Pay in Naira via Paystack

**What It Does:**
> Accept local payments through Paystack, bank transfer, or USSD. Your Nigerian debit card works perfectly—no international gateway drama.

**Why That Matters:**
> Launch campaigns immediately. No more "payment declined" messages. No begging friends for their dollar cards.

**Proof:**
> "I tried Wask for 3 months—payment failed every time. Tenzu accepted my GTBank card instantly." - Emeka A., Abuja Tech Vendor

[CTA]: Try Risk-Free →

### Example: Chat-Based Setup

**Feature:** AI Campaign Consultant

**What It Does:**
> Chat with our AI to build campaigns. It asks simple questions, explains every choice, and sets up your ads automatically.

**Why That Matters:**
> No confusing 50-field forms. No watching hours of tutorials. Just answer questions in plain English and launch.

**Proof:**
> "I built my first campaign in 2 minutes without watching a single tutorial. Even I was shocked." - Tunde B., Port Harcourt Food Vendor

[CTA]: Start Conversation →

## Objection-Handling FAQ Section

### Formula
```
[Question as Objection] → [Empathetic Acknowledgment] → [Direct Answer] → [Proof/Reassurance]
```

### Example 1: Trust Objection

**Q:** "Is Tenzu legit? I've been scammed by 'ad tools' before."

**Answer:**
> We get it—Nigeria has too many scammers promising ad magic. Here's the difference: Tenzu doesn't run your ads. You do. Through your own Meta Business account. We just make it easier. Your ad spend goes directly to Meta, not through us. We only charge ₦15k/month for access to the tool—no hidden fees, no percentage of your spend. Over 2,000 verified Nigerian businesses use Tenzu monthly.

**Proof:** [Display trust badges: Paystack Verified, 4.8★ Rating, 2000+ Active Users]

### Example 2: Payment Objection

**Q:** "Why should I trust Tenzu with my card details?"

**Answer:**
> You shouldn't! That's why we use Paystack—Nigeria's most trusted payment processor (they handle payments for Flutterwave, Piggyvest, and thousands of businesses). Your card details never touch our servers. Paystack handles everything, and they're PCI-DSS certified (the highest security standard). Plus, we offer bank transfer if you prefer zero online payment risk.

### Example 3: Capability Objection

**Q:** "Can I really create professional ads without design skills?"

**Answer:**
> Yes, and here's why: Our AI Studio doesn't just give you templates (like Canva). It actually redesigns your photos—removes messy backgrounds, improves lighting, generates lifestyle scenes. It's like having a professional photographer and designer on standby. Try it on one photo right now (no signup required).

[CTA]: Test AI Studio Free →

### Example 4: Comparison Objection

**Q:** "How is Tenzu different from Wask or AdCreative.ai?"

**Answer:**
> Three key differences:

> 1. **Payment:** Wask requires USD payment (often fails with Nigerian cards). Tenzu accepts Naira via Paystack.
> 2. **Creative:** Wask gives you templates. Tenzu's AI actually redesigns your photos.
> 3. **Focus:** Wask is built for US/EU marketers. Tenzu is built for Nigerian SMEs—WhatsApp links, mobile-first, local support.

> Bottom line: If you've ever seen "Payment Declined" on Wask, you need Tenzu.

## Social Proof Section

### Formula
```
[Specific Outcome] + [Timeframe] + [Named Source] + [Location/Niche]
```

### Strong Examples

```
"I went from ₦200k agency fees per campaign to ₦15k monthly unlimited. Tenzu paid for itself in week one."
— Chioma Okafor, Fashion Vendor, Lagos

"My first campaign got 2,300 WhatsApp messages in 48 hours. The AI targeting was scary accurate."
— Emeka Adeyemi, Phone Accessories, Abuja

"I've tried everything—Wask, Canva, hiring freelancers. Tenzu is the first tool my card actually accepts."
— Tunde Balogun, Food Delivery, Port Harcourt
```

### Weak Examples (Avoid These)

```
❌ "Tenzu is amazing!" — Anonymous
❌ "Best tool ever" — Happy Customer
❌ "I love Tenzu" — User
```

**Why They're Weak:**
- No specific outcome
- No named source
- No verifiable context
- Generic praise

## Pricing Section

### Formula
```
[Price] + [What You Get] + [Comparison Anchor] + [Risk Reversal]
```

### Example

**Heading:** One Price. Unlimited Campaigns.

**Price Display:**
```
₦15,000/month
Pay with your Nigerian card—no international gateway drama.
```

**What You Get:**
- ✅ Unlimited campaign launches
- ✅ AI Creative Studio (unlimited designs)
- ✅ AI Campaign Consultant (unlimited chats)
- ✅ WhatsApp & Email support
- ✅ Performance analytics dashboard

**Comparison Anchor:**
> Traditional agency: ₦150,000 per campaign (1× use)
> Tenzu: ₦15,000 per month (unlimited campaigns)

**Risk Reversal:**
> 7-day money-back guarantee. Cancel anytime—no questions asked.

[CTA]: Start Free Trial →

## CTA Section Variations

### Urgency-Based
```
Your Competitors Are Already Running Ads
Launch Your First Campaign in 2 Minutes

[CTA Button]: Start Free Trial →
[Subtext]: No credit card required. Cancel anytime.
```

### Outcome-Based
```
Join 2,000+ Nigerian Businesses Growing with Tenzu
Create professional ads. Pay in Naira. Launch in minutes.

[CTA Button]: Create My First Ad →
[Subtext]: Free 7-day trial. No payment required.
```

### Risk-Reversal-Based
```
Try Tenzu Risk-Free for 7 Days
If it doesn't work for your business, get a full refund—no questions asked.

[CTA Button]: Start Free Trial →
[Subtext]: Money-back guarantee. Cancel anytime.
```

## Complete Landing Page Example

### Meta Description
```
Run professional Facebook ads in 2 minutes. Pay in Naira via Paystack. AI builds campaigns for you. No design skills needed. Try free for 7 days.
```

### Hero Section
```
Run Professional Facebook Ads in 2 Minutes
Pay in Naira. No Expert Needed. No Dollar Card Drama.

Over 2,000 Nigerian vendors launch campaigns daily with Tenzu's AI Consultant.

[Primary CTA]: Start Free Trial →
[Secondary CTA]: See How It Works ↓
```

### Problem Section
```
Tired of "Payment Declined"?

You've tried Wask. You've tried AdCreative.ai. Same story: Your Nigerian card doesn't work. Their "international gateway" rejects you.

Meanwhile, your competitors—the ones using Tenzu—are launching campaigns right now.
```

### Solution Section
```
Tenzu: Built for Your Card, Your Business, Your Market

✅ Naira Payment: Paystack-powered. Your local card works perfectly.
✅ AI Creative Studio: Turn phone photos into professional ads in seconds.
✅ Chat-Based Setup: No confusing dashboards. Just simple questions.
✅ WhatsApp Integration: Most Nigerians don't have websites. We get it.

[CTA]: Try It Free for 7 Days →
```

### Features Grid
```
[3-column layout]

AI Creative Studio
Transform product photos into studio-quality ads with AI backgrounds, lighting fixes, and design enhancements.
[Mini CTA]: See Examples →

AI Campaign Consultant
Chat to build campaigns. AI handles targeting, budgets, and placements—you just answer questions.
[Mini CTA]: Try a Demo Chat →

Naira-Native Payment
Pay via Paystack, bank transfer, or USSD. Zero payment friction. Your card works here.
[Mini CTA]: View Pricing →
```

### Social Proof
```
Trusted by 2,000+ Nigerian Businesses

[3 testimonials with photos]
[Trust badges: Paystack Verified, 4.8★ Rating, 2000+ Users]
```

### Pricing
```
One Simple Price. Unlimited Campaigns.

₦15,000/month
vs. Agency fees: ₦150k per campaign

[Feature list]
[Risk reversal: 7-day money-back guarantee]

[CTA]: Start Free Trial →
```

### Final CTA
```
Your Competitors Launched 3 Campaigns This Week
Start Yours in 2 Minutes

[CTA Button]: Create My First Ad Now →
[Trust elements below CTA: No credit card • Cancel anytime • 7-day guarantee]
```

## Conversion Optimization Checklist

Before publishing any landing page, verify:
- [ ] Hero promise is outcome-focused (not feature-focused)
- [ ] Specific pain point is addressed in first 100 words
- [ ] Social proof includes names, locations, and specific outcomes
- [ ] CTAs are action-verbs (not generic "Learn More")
- [ ] Naira pricing is prominent (no hidden USD)
- [ ] Mobile experience is tested (most traffic is mobile)
- [ ] WhatsApp contact is visible (preferred support channel)
- [ ] Risk reversal is clear (guarantee, cancel policy)
- [ ] No jargon or technical terms without explanation
- [ ] Every claim has a proof point (testimonial, stat, screenshot)
