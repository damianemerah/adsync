# Email Campaign Sequences

Proven email templates and automation sequences for Tenzu marketing.

## General Email Guidelines

### Subject Line Rules
- **Length:** 40-50 characters (mobile preview)
- **Personalization:** Use first name when possible
- **Clarity over cleverness:** Direct benefit statements
- **Numbers work:** "3 ways to...", "2-minute setup"
- **Questions engage:** "Still paying in dollars?"

### Body Structure
- **Greeting:** Personal but professional
- **Hook:** First sentence must grab (pain or benefit)
- **Body:** 2-3 short paragraphs max
- **CTA:** One clear action, button format
- **Signature:** Name + role + support contact

### Tone
- Conversational but professional
- Second person ("you", not "we")
- Active voice
- Short sentences (10-15 words average)

## Welcome Sequence (Post-Signup)

### Email 1: Welcome + Quick Win (Send immediately)

**Subject:** Welcome to Tenzu, [Name]! Your first campaign in 2 minutes

**Body:**
```
Hi [Name],

Welcome to Tenzu! You're now part of 2,000+ Nigerian businesses running professional Facebook ads without the headaches.

Here's what happens next:

1. Connect your Meta Business account (30 seconds)
2. Chat with our AI to build your first campaign (2 minutes)
3. Upload a product photo—AI will make it look professional
4. Launch and watch the leads roll in

The fastest way to see Tenzu's power? Launch a simple campaign today. Even a small ₦5,000 test budget will show you the quality of leads.

Ready to start?

[CTA Button]: Build My First Campaign →

Questions? Reply to this email or WhatsApp us: +234 XXX XXX XXXX

— 
Chisom
Head of Growth, Tenzu
```

### Email 2: Education (Day 2)

**Subject:** The #1 mistake new ad managers make (and how to avoid it)

**Body:**
```
Hi [Name],

Quick question: Have you launched your first campaign yet?

If not, no stress. Most people hesitate because they're worried about wasting money on the wrong audience.

Here's the truth: The #1 mistake isn't bad targeting—it's bad creatives.

Even perfect targeting fails if your ad looks amateur. That's why we built AI Studio.

Here's what it does:
→ Removes messy backgrounds from product photos
→ Enhances lighting and colors
→ Generates lifestyle scenes (your product in use)
→ Creates 5 variations in 30 seconds

Try it now with any photo—even an old one from your phone:

[CTA Button]: Test AI Studio →

P.S. - Unlike Wask's templates, our AI actually redesigns your photo. Big difference.

— 
Chisom
```

### Email 3: Social Proof (Day 4)

**Subject:** "I got 2,300 WhatsApp messages in 48 hours" - Emeka, Abuja

**Body:**
```
Hi [Name],

I want to share a quick win from one of our users:

Emeka runs a phone accessories business in Abuja. Before Tenzu, he was paying an agency ₦150k per campaign.

His first Tenzu campaign:
- Budget: ₦15,000
- Duration: 48 hours
- Results: 2,300 WhatsApp messages
- Cost per message: ₦6.50

His exact words: "The AI targeting was scary accurate. I thought it would take weeks to learn this."

The best part? He built that campaign in 2 minutes using our AI Consultant. No prior experience.

Want similar results?

[CTA Button]: Launch My Campaign Now →

Real user, real results. See more case studies here: [link]

— 
Chisom
```

### Email 4: Objection Handling (Day 7)

**Subject:** "Is Tenzu actually different from Wask?"

**Body:**
```
Hi [Name],

I've noticed you haven't launched a campaign yet. That's totally fine—Tenzu isn't for everyone.

But I want to address the #1 question we get: "How is this different from Wask?"

Three key differences:

1. Payment: Wask requires USD payment (fails with most Nigerian cards). Tenzu uses Paystack—your local card works perfectly.

2. Creative: Wask gives you templates. Tenzu's AI actually redesigns your photos (removes backgrounds, improves lighting, generates scenes).

3. Focus: Wask is built for US/EU desktop users. Tenzu is built for Nigerian mobile-first businesses.

Bottom line: If you've ever seen "Payment Declined" on Wask, you need Tenzu.

Still unsure? Reply with your biggest concern and I'll address it personally.

[CTA Button]: Start My Free Trial →

— 
Chisom
```

## Re-Engagement Sequence (Inactive Users)

### Email 1: Gentle Nudge (14 days inactive)

**Subject:** [Name], we noticed you haven't launched yet

**Body:**
```
Hi [Name],

It's been 2 weeks since you joined Tenzu, but I notice you haven't launched a campaign yet.

What's holding you back?

→ Not sure how to start? (Our AI Consultant guides you step-by-step)
→ Worried about wasting money? (Start with ₦5k test budget)
→ Don't have good photos? (AI Studio fixes that)
→ Confused by targeting? (AI handles it for you)

Whatever it is, I want to help. Reply to this email with your concern and I'll sort it out personally.

Or just start with a simple test campaign:

[CTA Button]: Build Campaign in 2 Minutes →

— 
Chisom
```

### Email 2: Payment Win-Back (21 days inactive)

**Subject:** Card declined? Here's why Tenzu is different

**Body:**
```
Hi [Name],

I'm going to guess why you haven't launched: Payment issues with other tools?

You're not alone. 60% of Nigerian cards get rejected by international gateways (Wask, Shopify, Canva Pro, etc.).

That's actually why we built Tenzu.

Unlike those tools, we use Paystack—the payment processor trusted by Piggyvest, Flutterwave, and thousands of Nigerian businesses.

Your local card works perfectly. Zero friction.

Don't believe me? Try adding your payment method right now:

[CTA Button]: Add My Card →

If it fails (it won't), email me directly and I'll refund any test charge immediately.

— 
Chisom
```

### Email 3: Competitor Comparison (28 days inactive)

**Subject:** Wask vs. Tenzu: An honest comparison

**Body:**
```
Hi [Name],

I know you're probably comparing tools right now. Smart move.

Here's an honest breakdown of Tenzu vs. Wask:

**Where Wask Wins:**
→ More template options
→ Desktop experience is slightly more polished
→ Brand recognition (they've been around longer)

**Where Tenzu Wins:**
→ Payment: Your Nigerian card actually works (Paystack)
→ Creative: AI redesigns your photos (not just templates)
→ Setup: Chat-based (not 50-field forms)
→ Focus: Built specifically for Nigerian SMEs

**The Real Question:**
Do you want a tool that looks pretty but rejects your payment? Or one that actually works for Nigerian businesses?

Try Tenzu free for 7 days. If Wask is better for you, keep using Wask. No hard feelings.

[CTA Button]: Start Free Trial →

— 
Chisom

P.S. - Our cancellation process is literally one click. No retention calls. No dark patterns.
```

## Payment Recovery Sequence

### Email 1: Immediate Payment Failure

**Subject:** Oops - Payment failed. Let's fix this together

**Body:**
```
Hi [Name],

Your payment just failed. Don't worry—this happens all the time with international gateways.

Two quick fixes:

**Option 1: Bank Transfer (Instant)**
→ Transfer ₦15,000 to our Paystack account
→ Use reference: [unique_id]
→ Access restored in 2 minutes

[CTA]: See Transfer Details →

**Option 2: Different Card**
→ Try another card (sometimes one works when others don't)
→ Paystack accepts all Nigerian bank cards

[CTA]: Update Payment Method →

Need help? WhatsApp me directly: +234 XXX XXX XXXX

— 
Chisom
```

### Email 2: Post-Subscription Cancellation

**Subject:** We'll miss you, [Name]. One quick question?

**Body:**
```
Hi [Name],

I noticed you just cancelled your Tenzu subscription. That's totally fine—no hard feelings.

But I have one quick question: What made you leave?

→ Payment issues?
→ Features not working as expected?
→ Too expensive?
→ Found a better alternative?
→ Something else?

Just reply with a quick note. Your feedback genuinely helps us improve.

P.S. - If you cancelled because of a specific issue, let me know. I might be able to fix it immediately and get you back on track.

— 
Chisom
```

## Launch Success Sequence (Post-First Campaign)

### Email 1: Congratulations (Immediately after launch)

**Subject:** 🎉 Your campaign is live! Here's what to watch

**Body:**
```
Hi [Name],

Congrats! Your first Tenzu campaign just went live.

Here's what to expect in the next 24 hours:

**Hour 1-2: Learning Phase**
→ Meta is testing your ad with different audiences
→ Spend will be slow (this is normal)
→ Don't panic if you see zero results yet

**Hour 3-12: Optimization Phase**
→ Meta finds your best audience
→ Cost per result stabilizes
→ Leads should start coming in

**Hour 13-24: Full Speed**
→ Campaign running at full capacity
→ Consistent lead flow
→ Time to prepare for messages!

Track performance in real-time:

[CTA Button]: View Campaign Dashboard →

Questions? I'm here: [email/WhatsApp]

— 
Chisom

P.S. - Most campaigns hit their stride after 24 hours. Give it time.
```

### Email 2: Optimization Tips (Day 2)

**Subject:** Your campaign results + 3 quick optimization tips

**Body:**
```
Hi [Name],

Your campaign has been live for 2 days. Here's what I'm seeing:

**Your Stats:**
→ Impressions: [X]
→ Clicks: [X]
→ Messages: [X]
→ Cost per message: ₦[X]

**3 Quick Wins:**

1. **If cost per message > ₦20:** Try narrowing your audience in Tenzu AI > "Edit Targeting"

2. **If clicks are high but messages are low:** Your product pricing might be too high for cold traffic. Consider a lower-price "entry offer."

3. **If impressions are low:** Increase daily budget by ₦2,000-3,000 to speed up delivery.

Want help optimizing? Reply with your campaign link and I'll analyze it.

[CTA Button]: Edit My Campaign →

— 
Chisom
```

## Content & Education Emails

### Monthly Newsletter Template

**Subject:** February Tenzu Insider: 3 campaign strategies that worked this month

**Body:**
```
Hi [Name],

Happy February! Here's what's working in Nigerian Facebook ads right now:

**1. WhatsApp Click-to-Chat Campaigns**
→ Strategy: Use instant forms with auto-WhatsApp redirect
→ Why it works: Friction is low (one click to chat)
→ Best for: Service businesses, consultations, high-ticket items

**2. Video Testimonials**
→ Strategy: Record happy customers (even on phone)
→ Why it works: Trust is everything in Nigeria
→ Best for: Any business with repeat customers

**3. Flash Sale Countdown**
→ Strategy: 48-hour sale with countdown timer in creative
→ Why it works: Creates urgency without being scammy
→ Best for: Fashion, gadgets, consumables

Want to try any of these? Hit reply and I'll walk you through setup.

[CTA Button]: Try a New Strategy →

— 
Chisom

P.S. - New AI Studio feature dropping next week. Stay tuned.
```

## Support & Retention Emails

### Proactive Support (When User Struggles)

**Subject:** I noticed your campaign paused—everything okay?

**Body:**
```
Hi [Name],

I noticed your campaign paused after 3 days. Just checking in—is everything alright?

Common reasons campaigns pause:
→ Budget exhausted (easy fix: increase budget)
→ Ad rejection by Meta (we can help fix creative)
→ Poor performance (let's optimize together)

Whatever it is, I'm here to help. Reply with what happened and I'll sort it out.

[CTA Button]: Reactivate Campaign →

— 
Chisom
```

### Feature Announcement

**Subject:** New: AI Studio now generates lifestyle scenes

**Body:**
```
Hi [Name],

Quick update: AI Studio just got a massive upgrade.

**What's New:**
Our AI can now generate entire lifestyle scenes—showing your product being used in real contexts.

**Example:**
→ Upload: Photo of your handbag on a white background
→ AI creates: Handbag on a café table, held by a woman in Lagos traffic, displayed in a boutique, etc.

→ All in 30 seconds.

**Why This Matters:**
Lifestyle scenes convert 3x better than product-only photos. Now you can create them without a photographer or model.

Try it free:

[CTA Button]: Generate Lifestyle Scene →

— 
Chisom
```

## Email Template Checklist

Before sending any email, verify:
- [ ] Subject line is clear and benefit-focused
- [ ] First sentence hooks (pain or benefit)
- [ ] Body is scannable (short paragraphs, bullet points)
- [ ] One clear CTA (button format)
- [ ] Mobile-optimized (test on phone)
- [ ] Signature includes support contact
- [ ] Tone matches recipient stage (new vs. churned vs. active)
- [ ] No jargon without explanation
- [ ] Specific, not generic ("2 minutes" not "fast")
- [ ] Localized (Naira, WhatsApp, Nigerian context)
