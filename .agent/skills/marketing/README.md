...
│   ├── landing-pages.md               # Landing page copy formulas & examples
│   ├── email-sequences.md             # Email campaign templates
│   ├── social-templates.md            # Platform-specific social media copy
│   └── battle-cards.md                # Competitive positioning & objection handling
└── examples/
    └── usage-examples.md              # Practical usage scenarios
```
...
1. **Read SKILL.md first** - Get overview of the three pillars and core framework
2. **Identify your context** - What are you creating? (landing page, email, social post, etc.)
3. **Reference specific file** - Use the quick reference links in SKILL.md
4. **Apply frameworks** - Use the formulas and templates provided
5. **Check guidelines** - Verify against brand voice and anti-scam filter
...
- Training sales team
- Handling "Why not Wask?" objections
...
- [ ] Tone matches audience stage (new/churned/active)
- [ ] CTA is specific and single (not multiple)
- [ ] Anti-scam filter passed (no get-rich-quick)
- [ ] Competitive positioning honest (acknowledge strengths)
- [ ] Mobile-optimized (most users on phones)
- [ ] Scannable format (short paragraphs, bullets where appropriate)
- [ ] Proof point included (testimonial, stat, screenshot)
...
3. Claude will reference the skill when you ask for:
   - Landing page copy
   - Email campaigns
   - Social media posts
...
- Adding new competitive battle cards for emerging competitors
...
1. **Start with SKILL.md** - Don't jump directly to reference files
2. **Use progressive disclosure** - Only read reference files when needed
3. **Apply frameworks** - Don't write from scratch; adapt templates
4. **Check brand voice** - Every piece should pass vocabulary test
5. **Test competitively** - Always position against specific alternatives
6. **Localize context** - Nigerian realities must be acknowledged
7. **Verify mobile-first** - Most users are on phones
8. **Include proof** - Never make claims without evidence
...
- Pricing changes (update all references)
- Market positioning shifts (revise pillars in SKILL.md)
...
4. Review battle-cards.md for competitive positioning
...
- Example files for practical guidance
- Appropriate degree of freedom for creative tasks
