---
created: Jan 23, 2026, 12:42 AM
title: Introduction to Fabric.js. Part 2. — Fabric.js Javascript Canvas Library
tags:
source: https://fabric5.fabricjs.com/fabric-intro-part-2#events
description: Introduction to Fabric.js. Part 2. — Fabric.js Javascript Canvas Library
author:
published:
---
...
In the [first part of this series](https://fabric5.fabricjs.com/fabric-intro-part-1) , we only started to get familiar with Fabric.js. We looked at the reasons to use Fabric, at its object model and object hierarchy, at different kind of entities available in Fabric — simple shapes, images, and complex paths. We also learned how to perform simple operations with Fabric objects on canvas.
...
- `object.dispose()` - all animations created by the object ( `object.animate(...)` ) are cancelled.
  If you want to add animations using
  `fabric.util.animate` instead of `object.animate(...)` you can attach them to an object by passing the
  `target` property. This way the animations will cancel once the object is disposed.
...
![](https://fabric5.fabricjs.com/article_assets/2_1.png)
...
![](https://fabric5.fabricjs.com/article_assets/2_2.png)
...
![](https://fabric5.fabricjs.com/article_assets/2_3.png)
...
![](https://fabric5.fabricjs.com/article_assets/2_4.png)
...
![](https://fabric5.fabricjs.com/article_assets/2_15.png)
In the above example, we create a circle at 100,100 location, with 50px radius.
We then set its fill to a gradient that spans throughout entire height of that circle, from black to white.
Gradient option object has 2 main properties, coords and colorStops. `coords` expect at least 2 coordinates
pairs (x1, y1 and x2, y2) that will define how the gradient extend on the object, while `colorStops` is an array
that defines the gradients colors. Each color in the array is defined by an `offset` that represents its position
on the gradient, a `color` that defines the color itself and eventually an `opacity` property.
...
Coordinates are relative to object top left corner, so the highest point of the circle is `0` , and the lowest point is `circle.height` .
\
 You can specify type `linear` or `radial` to obtain 2 different kind of gradients, and also
`gradientUnits` defaults to pixels but can be specified as `percentage`. `percentage` will allow the gradient
size to be specified in percentage of object size, with `1` being 100% of object size.\
...
![](https://fabric5.fabricjs.com/article_assets/2_16.png)
...
![](https://fabric5.fabricjs.com/article_assets/2_17.png)
...
![](https://fabric5.fabricjs.com/article_assets/2_5.png)
...
![](https://fabric5.fabricjs.com/article_assets/2_6.png)
...
![](https://fabric5.fabricjs.com/article_assets/2_7.png)
...
![](https://fabric5.fabricjs.com/article_assets/2_8.png)
...
![](https://fabric5.fabricjs.com/article_assets/2_9.png)
...
![](https://fabric5.fabricjs.com/article_assets/2_10.png)
...
![](https://fabric5.fabricjs.com/article_assets/2_11.png)
...
![](https://fabric5.fabricjs.com/article_assets/2_12.png)
...
![](https://fabric5.fabricjs.com/article_assets/2_13.png)
...
![](https://fabric5.fabricjs.com/article_assets/2_14.png)
...
Check this [events demo](https://fabric5.fabricjs.com/events) for a more extensive exploration of Fabric's event system.
Read [Part 3](https://fabric5.fabricjs.com/fabric-intro-part-3) .
