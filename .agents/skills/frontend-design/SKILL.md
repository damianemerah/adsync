...
## ⚡ Tenzu Project Override

> **Design context is pre-loaded for this project. Skip teach-impeccable.**
> Context lives in `.impeccable.md` at the project root. Read it before every task.

This is **Tenzu** — an ad management platform for Nigerian SMEs who sell via WhatsApp.
The design system is locked. These rules override the generic Impeccable guidelines below:

### Locked Constraints (Non-Negotiable)

- **Tokens only**: Never use hex codes, `bg-blue-500`, or arbitrary `bg-[#xyz]` in JSX. Every color must come from `src/app/globals.css` semantic tokens.
- **Font lock**: Use `font-heading` (Montserrat Alternates) for h1–h3, `font-sans` (Montserrat) for everything else. Do NOT suggest Inter, Roboto, or system fonts.
- **Flat UI**: Depth = `border border-border`. Never `shadow-md`, `shadow-lg` on surfaces.
- **Radius**: `rounded-lg` for containers, `rounded-md` for inputs. `rounded-3xl` is banned.
- **AI = Purple**: `text-ai` / `bg-ai` for ALL AI features. Always. No exceptions.
- **Icons**: `iconoir-react` only. Never `lucide-react`.
- **Mobile-first**: 375px is the real viewport. Every layout is mobile-first.

### Context Gathering Protocol (Tenzu shortcut)

1. Read `.impeccable.md` at the project root — it contains the full Design Context, token reference, and anti-pattern table.
2. Proceed immediately. Do NOT run `/teach-impeccable` — context is already established.
