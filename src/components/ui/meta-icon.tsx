...
interface MetaIconProps {
  className?: string;
  /** Use the gradient version (default) or a flat monochrome version */
  mono?: boolean;
}

/**
 * Official Meta "M" infinity-loop mark.
 * Source: https://about.meta.com/brand/resources/meta/meta-mark/
 *
 * Usage:
 *   <MetaIcon className="h-5 w-5" />           ← gradient (default)
 *   <MetaIcon className="h-5 w-5" mono />      ← currentColor, inherits text-* class
 */
export function MetaIcon({ className, mono = false }: MetaIconProps) {
  const id = "meta-grad"; // stable — only one gradient definition needed per SVG

  if (mono) {
    return (
      <svg
        viewBox="56.33 457.15 2387.34 1585.74"
      ...
      >
        ...
        <path d="M740 716.43c-101.86 0-188.37 71.48-260.76 180.82-102.37 154.5-165 384.63-165 605.63 0 91.16 20 161.14 46.16 203.48l-219.89 144.79c-56.45-93.89-84.18-217.1-84.18-357.48 0-255.29 70.07-521.37 203.32-726.85 118.22-182.23 288.83-309.67 484.51-309.67z" />
      </svg>
    );
  }
