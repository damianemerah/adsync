...
const RECOVERY_CHIP_TEMPLATES = [
  { label: "👗 Fashion & Clothing", template: "I sell fashion and clothing in {location}" },
  { label: "💄 Beauty & Skincare", template: "I sell skincare and beauty products in {location}" },
  { label: "🍖 Food & Catering",   template: "I sell food and catering services in {location}" },
  { label: "💇 Wigs & Hair",       template: "I sell wigs and hair products in {location}" },
  { label: "📱 Electronics",       template: "I sell phones and electronics in {location}" },
  { label: "🎂 Cakes & Pastries",  template: "I sell cakes and pastries in {location}" },
];
      ...
      <div className="p-4 rounded-2xl bg-ai/5 border border-ai/20 space-y-3">
        <div className="flex items-start gap-2">
          ...
          <div className="space-y-1">
            <p className="text-sm font-semibold text-foreground">Your edit instruction</p>
            <p className="text-xs text-muted-foreground italic">“{editInstruction}”</p>
          </div>
        </div>
        <p className="text-xs text-foreground/70 leading-relaxed">
          Structural edits like adding props or swapping objects work best in Studio, 
          where you can paint exactly what to change. Your instruction is already pre-filled.
        </p>
      </div>
      <Button
      ...
      {showConstraintInput && (
        <div className="flex gap-2 animate-in fade-in slide-in-from-top-1">
          <input
            ...
            onKeyDown={(e) => { if (e.key === "Enter") handleRegenWithConstraint(); }}
...
export function ChatBubble({
  ...
  return (
    <div
      className={cn(
        "flex gap-3 max-w-[92%] animate-in fade-in slide-in-from-bottom-2",
      ...
      <div className="space-y-2 min-w-0">
        ...
        {message.type === "network_error" && (
          <div className="mt-3 animate-in fade-in slide-in-from-top-2">
            <button
              onClick={() => onRecoverySelect(message.data?.originalInput || "")}
        ...
        {message.type === "recovery" && !message.data?.clarificationOptions?.length && (
          <div className="mt-3 space-y-2 animate-in fade-in slide-in-from-top-2">
            <div className="flex flex-wrap gap-2">
              {buildRecoveryChips(message.data?.detectedLocation).map((chip) => (
                <button
                  key={chip.value}
        ...
        {message.type === "studio_suggestion" && message.data?.generatedImage && (
        ...
        {message.type === "suggestion" &&
          ...
          !message.data.outcomeLabel && (
            <div className="bg-card border border-border rounded-xl p-4 shadow-sm space-y-4 animate-in fade-in slide-in-from-top-2">
              <div className="flex flex-wrap gap-2">
                {message.data.interests?.map((int: any) => {
