...
/**
 * Category-scoped life event catalog for prompt injection.
 * Returns relevant life events first, padded with universal entries up to `max`.
 */
export function buildScopedLifeEventCatalogPrompt(
  category?: string,
  max = 15,
): string {
  if (!category) return buildLifeEventCatalogPrompt();

  const relevant = suggestLifeEventsForCategory(category);
  ...
  const relevantIds = new Set(relevant.map((e) => e.name));
  const rest = META_LIFE_EVENT_SEEDS.filter(
    (e) => !relevantIds.has(e.name),
  ).sort((a, b) => b.relevantFor.length - a.relevantFor.length);

  const combined = [...relevant, ...rest].slice(0, max);
  return combined.map((e) => e.name).join(" | ");
}
