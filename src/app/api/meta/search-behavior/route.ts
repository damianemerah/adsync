...
async function remapWithAI(originalQuery: string): Promise<string | null> {
  ...
  try {
    const catalog = buildBehaviorCatalogPrompt();
    const res = await openai.responses.create({
      model: "gpt-4o-mini",
      input: `Map this AI-generated Meta Ads behavior name to the closest valid catalog entry.
...
Reply with ONLY the exact catalog name, or the word "none".`,
    ...
    return buildBehaviorCatalogPrompt().split(" | ").includes(answer)
      ? answer
      : null;
  } catch {
    return null;
...
export async function GET(request: Request) {
  ...
  try {
    ...
    let results = await MetaService.searchBehaviors(
      accessToken,
    ...
    if (!hasExact && !resolveLocalBehavior(query)?.metaId) {
      const remapped = await remapWithAI(query);
      if (remapped && remapped !== query) {
        console.log(
          `[search-behavior] gpt-4o-mini remapped "${query}" → "${remapped}"`,
        );
        const retryResults = await MetaService.searchBehaviors(
          accessToken,
          adAccount.platform_account_id,
          remapped,
        );
